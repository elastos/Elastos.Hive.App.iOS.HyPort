...
@objc public enum SuspendOpenAnimationType: Int {
  case pop
  case fade
  case slideLeft
  case slideUp
...
@objc public enum SuspendVerticalDirection: Int {
  case up
  case down
}

/**
 Suspend Object. It has `SuspendItem` objects.
 Suspend support storyboard designable.
 */
@IBDesignable
open class Suspend: UIView {
  // MARK: - Properties
  
  /**
   `SuspendItem` objects.
   */
  @objc open var items: [SuspendItem] = []
  ...
  /**
   Whether or not suspend responds to keyboard notifications and adjusts its position accordingly
   */
  @objc @IBInspectable
  open var respondsToKeyboard: Bool = true
  
  @objc open var openAnimationType: SuspendOpenAnimationType = .pop
  
  @objc open var verticalDirection: SuspendVerticalDirection = .up
  
  @objc open var friendlyTap: Bool = true
  
  @objc open var sticky: Bool = false
    
  public static var global: SuspendManager {
    get {
      return SuspendManager.defaultInstance()
    }
  }
   ...
   */
  @IBOutlet open weak var fabDelegate: SuspendDelegate?
  ...
  */
  ...
  @objc open func open() {
    ...
    if let shouldOpen = fabDelegate?.suspendShouldOpen?(self), shouldOpen == false {
      return
    }

    fabDelegate?.suspendWillOpen?(self)
  ...
  @objc open func close() {
    if let shouldClose = fabDelegate?.suspendShouldClose?(self), shouldClose == false {
      return
    }

    fabDelegate?.suspendWillClose?(self)
    let animationGroup = DispatchGroup()
    ...
    animationGroup.notify(queue: .main) {
      self.fabDelegate?.suspendDidClose?(self)
    }
    
    circleLayer.backgroundColor = self.buttonColor.cgColor
    
    fabDelegate?.suspendClosed?(self)
    closed = true
  }
  ...
  @objc open func toggle() {
    if items.count > 0 {
      ...
      fabDelegate?.emptySuspendSelected?(self)
    }
  }
  ...
  /**
   Add custom item
   */
  @objc open func addItem(item: SuspendItem) {
   ...
   */
  @discardableResult
  @objc open func addItem(title: String, titlePosition: SuspendItemLabelPositionType = .left) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
   ...
   */
  @discardableResult
  @objc open func addItem(title: String) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.title = title
    addItem(item: item)
   ...
   */
  @discardableResult
  @objc open func addItem(_ title: String, icon: UIImage?, titlePosition: SuspendItemLabelPositionType = .left) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.titleLabelPosition = titlePosition
    item.title = title
   ...
   */
  @discardableResult
  @objc open func addItem(_ title: String, icon: UIImage?) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.title = title
    item.icon = icon
   ...
   */
  @discardableResult
  @objc open func addItem(title: String, handler: @escaping ((SuspendItem) -> Void)) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.title = title
   ...
   */
  @discardableResult
  @objc open func addItem(title: String, titlePosition: SuspendItemLabelPositionType = .left, handler: @escaping ((SuspendItem) -> Void)) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.titleLabelPosition = titlePosition
    item.title = title
   ...
   */
  @discardableResult
  @objc open func addItem(_ title: String, icon: UIImage?, handler: @escaping ((SuspendItem) -> Void)) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.title = title
    item.icon = icon
   ...
   */
  @discardableResult
  @objc open func addItem(_ title: String, icon: UIImage?, titlePosition: SuspendItemLabelPositionType = .left, handler: @escaping ((SuspendItem) -> Void)) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.titleLabelPosition = titlePosition
    item.title = title
   ...
   */
  @discardableResult
  @objc open func addItem(icon: UIImage?) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.icon = icon
    addItem(item: item)
   ...
   */
  @discardableResult
  @objc open func addItem(icon: UIImage?, handler: @escaping ((SuspendItem) -> Void)) -> SuspendItem {
    let item = SuspendItem()
    itemDefaultSet(item)
    item.icon = icon
    item.handler = handler
  ...
  /**
   Remove item.
   */
  @objc open func removeItem(item: SuspendItem) {
  ...
  fileprivate func determineTapArea(item : SuspendItem) -> CGRect {
  ...
  fileprivate func itemDefaultSet(_ item: SuspendItem) {
  ...
  fileprivate func setBottomFrameAccordingToRTL(_ keyboardSize: CGFloat = 0) {
    ...
    if SuspendManager.defaultInstance().rtlMode {
